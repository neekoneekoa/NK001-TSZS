...
### 引脚分配
- PB13-PB15：3个LED指示灯，分别表示系统状态（正常/错误/警告）
- PB1 PB0 PB11 PB10  按键
- PB8-9 OLED显示
- PA8 蜂鸣器
- PA9 PA10  串口
- PA11 PWM调节
...
- 主界面：标题---运行时间---图案
- PWM界面：标题--频率调整--占空比调整--启动/停止PWM
- ADC界面：标题--显示采样值--启动ADC通道--那些通道
- 串口界面：标题--波特率设置--数据位设置--校验位设置--停止位设置--发送/接收数据
- GPIO输入界面：标题--显示引脚状态--切换要显示的引脚--下一个引脚
- GPIO输出界面：标题--显示引脚状态--切换引脚状态--下一个引脚
- 自由调试界面：显示接下来的4个 任务，右边显示对应任务触发条件（KEY还是时间还是 其他）
